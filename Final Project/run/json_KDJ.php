...
$sql="SELECT Date,K,D,J FROM HistoricalPrices WHERE Symbol=\"{$name['Symbol']}\" ORDER BY `Date` ASC";  
...
{
    $i=0; 
	$fp=fopen("json/KDJ/{$name['Symbol']}.json","w");
	fwrite($fp,"[");
    ...
    { 
		...
		$str="[".$row['Date'].",".$row['K'].",".$row['D'].",".$row['J']."],";
		if ($i!=$countt['count(*)']-1) $str="[".$row['Date'].",".$row['K'].",".$row['D'].",".$row['J']."],";
		else  $str="[".$row['Date'].",".$row['K'].",".$row['D'].",".$row['J']."]";
			fwrite($fp,$str);
			fwrite($fp,"\n");
    } 
